...
// NOTE: bumpUsage operates on a short-lived in-memory cache flushed every 5s.
// Key regeneration / admin writes bypass the cache (they call writeUsers
// directly), so we invalidate the cache on every external write below.
let _dirty = false;
let _cache = null;
function invalidateCache() { _cache = null; _dirty = false; }
...
function flushLoop() {
  _timer = setTimeout(() => {
    ...
    if (_dirty) {
      try {
        // re-read + merge so we never clobber concurrent admin/regen writes
        const fresh = readUsers();
        for (const cu of _cache || []) {
          const fu = fresh.find(x => x.id === cu.id);
          if (fu && fu.key === cu.key) { fu.requests = cu.requests; fu.tokens = cu.tokens; }
        }
        writeUsers(fresh);
      } catch (_) {}
      _dirty = false; _cache = null;
    }
  }, 5000);
